...
--
-- Structure for view `Saldo_Nasabah_View`
--
DROP TABLE IF EXISTS `Saldo_Nasabah_View`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `Saldo_Nasabah_View`  AS SELECT `Nasabah`.`id_nasabah` AS `id_nasabah`, `Nasabah`.`nama` AS `nama`, `Nasabah`.`saldo` AS `saldo` FROM `Nasabah``Nasabah`  ;
